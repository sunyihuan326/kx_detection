...
# @Time    : 2020/7/23
# @Author  : sunyihuan
# @File    : ckpt_topn_dir.py
'''
某一文件夹下所有文件topn准确率，并输出excel表格
...
if __name__ == '__main__':
    ...
    img_root = "E:/WLS_originalData/3660camera_data202007/one_phase"  # 图片文件地址
    Y = YoloPredict()
    end_time0 = time.time()
    ...
    sheet1.write(0, 2, "top3_acc")
    sheet1.write(0, 3, "top5_acc")
    sheet1.write(0, 4, "top8_acc")
    sheet1.write(0, 5, "top10_acc")
    for c in clses:
        all_c_jpg=0
        acc_c_jpg=0
        ...
        if c not in ["nofood", "sweetpotatom", "potatom"]:
                ...
                if img.endswith("jpg"):
                    ...
                    if len(bboxes) == 0:
                        ...
                        if len(bboxes_pr) == 0:
                            ...
                            if classes_id22[c] in dict(best_bboxes_3).keys():
                                # food_topn_acc_b+=1
                                food_top3_cls_acc_nums += 1
                            if classes_id22[c] in dict(best_bboxes_5).keys():
                                # food_topn_acc_b+=1
                                food_top5_cls_acc_nums += 1
                            if classes_id22[c] in dict(best_bboxes_8).keys():
                                # food_topn_acc_b+=1
                                food_top8_cls_acc_nums += 1
                            if classes_id22[c] in dict(best_bboxes_10).keys():
                                # food_topn_acc_b+=1
                                food_top10_cls_acc_nums += 1
                            else:
                                print(classes_id22[c], dict(best_bboxes_10).keys())
            ...
            sheet1.write(clses.index(c) + 1, 2, food_top3_cls_acc_nums / all_c_jpg)
            sheet1.write(clses.index(c) + 1, 3, food_top5_cls_acc_nums / all_c_jpg)
            sheet1.write(clses.index(c) + 1, 4, food_top8_cls_acc_nums / all_c_jpg)
            sheet1.write(clses.index(c) + 1, 5, food_top10_cls_acc_nums / all_c_jpg)
    workbook.save("E:/WLS_originalData/3660camera_data202007/one_phase/top_n_m3_0717.xls")
    print("正确数：", acc_jpg)
