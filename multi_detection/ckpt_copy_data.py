...
if __name__ == '__main__':
    ...
    img_root = "F:/serve_data/202101-04/classes_others/new"  # 图片文件地址
    Y = YoloPredict()
    end_time0 = time.time()
    print("model loading time:", end_time0 - start_time)

    img_noresults = "F:/serve_data/202101-04/classes_others/noresults"
    if not os.path.exists(img_noresults): os.mkdir(img_noresults)  # 无任何结果文件夹

    img_nofood = "F:/serve_data/202101-04/classes_others/nofood"
    if not os.path.exists(img_nofood): os.mkdir(img_nofood)  # nofood文件夹
    ...
    for im in tqdm(os.listdir(img_root)):
        img_path = img_root + "/" + im
        ...
        # 食材分到对应文件夹
        if len(bboxes_pr) == 0:
            shutil.move(img_path, img_noresults + "/" + im)
        else:
            pre = int(bboxes_pr[0][-1])
            if pre == 9:  # 移动空文件
                shutil.move(img_path, img_nofood + "/" + im)
