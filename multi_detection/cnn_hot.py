...
class get_cam(object):
    ...
    def cam(self, sml_type):
        ...
        image_data = utils.image_preporcess(image, [320, 320])
        ...
        pred_bbox = np.concatenate([np.reshape(pred_sbbox, (-1, 5 + 22)),
                                    np.reshape(pred_mbbox, (-1, 5 + 22)),
                                    np.reshape(pred_lbbox, (-1, 5 + 22))], axis=0)

        best_bboxes = self.get_top_cls(pred_bbox, org_h, org_w, 2)  # 获取top_n类别和置信度

        heatmap_lbbox = np.zeros(pre_dict[sml_type].shape)
        classes = list(dict(best_bboxes).keys())[1]
        reader = Y.reader
        lbbox_w = reader.get_tensor("{}/weight".format(typ_dict[sml_type]))
        lbbox_w = lbbox_w.reshape((int(size_dict[sml_type]), 3, 27))
        lbbox_w = lbbox_w[:, :, 5 + classes]
        lbbox_w = np.sum(lbbox_w, 1)
        heatmap_lbbox = heatmap_lbbox + np.multiply(pre_dict[sml_type], lbbox_w)

        return heatmap_lbbox


if __name__ == "__main__":
    img_path = "C:/Users/sunyihuan/Desktop/test_all/20200617094642.jpg"
    Y = YoloPredict()
    ...
    cv2.imwrite('cam1.jpg', superimposed_img)
